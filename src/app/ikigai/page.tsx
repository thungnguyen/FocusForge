...
import { Compass, Sparkles, Plus, Trash2, HelpCircle } from "lucide-react";
import SectionCard from "@/components/ui/SectionCard";
import { IKIGAI_DATA } from "@/lib/mock-data";

export default function Ikigai() {
  const [love, setLove] = useState(IKIGAI_DATA.love);
  const [goodAt, setGoodAt] = useState(IKIGAI_DATA.goodAt);
  const [needs, setNeeds] = useState(IKIGAI_DATA.worldNeeds);
  const [paidFor, setPaidFor] = useState(IKIGAI_DATA.paidFor);
  const [summary, setSummary] = useState(IKIGAI_DATA.summary);
  ...
  const handleRemoveItem = (
    ...
    setVal((prev) => prev.filter((_, idx) => idx !== index));
  };

  return (
    <div className="space-y-6">
      {/* Intro Box in Vietnamese */}
      ...
      {/* Visual overlapping circles representation */}
      <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-workbook flex flex-col items-center justify-center space-y-4">
        ...
        <div className="relative w-56 h-56 md:w-64 md:h-64 flex items-center justify-center">
          ...
          <div className="absolute w-12 h-12 rounded-full bg-stone-900 border border-stone-800 flex items-center justify-center text-center p-1 z-10 shadow-md">
            <span className="text-[7px] font-black text-white uppercase tracking-widest leading-none">
              Ikigai
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <SectionCard 
          title={loveTitle} 
          onTitleChange={setLoveTitle}
          subtitle="Những điều khiến trái tim bạn rung động, mang lại niềm vui và động lực sống tự nhiên."
        >
          <div className="space-y-4">
            <ul className="space-y-2">
              {love.map((item, idx) => (
                <li
                  key={idx}
                  className="flex items-center justify-between p-2.5 bg-stone-50 border border-stone-100 rounded-lg text-base text-stone-700 font-handwriting"
                >
                  <span className="line-clamp-2 pr-2">{item}</span>
                  <button
                    onClick={() => handleRemoveItem(idx, setLove)}
                    className="p-1 rounded text-stone-400 hover:text-rose-600 cursor-pointer font-sans"
                  >
                    <Trash2 size={13} />
                  </button>
                </li>
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="Thêm điều bạn thích..."
                className="w-full bg-stone-50 border border-stone-200 rounded-lg px-2.5 py-1.5 text-base text-stone-800 font-handwriting focus:outline-none"
              />
              <button
                onClick={() => handleAddItem("love", newLove, setLove, setNewLove)}
                className="bg-stone-900 hover:bg-stone-800 text-stone-50 rounded-lg px-3 py-1.5 text-xs font-bold cursor-pointer font-sans"
              >
                Add
              </button>
            </div>
        ...
        {/* Card 2: What I Am Good At */}
        <SectionCard 
          title={goodAtTitle} 
          onTitleChange={setGoodAtTitle}
          subtitle="Những thế mạnh, kỹ năng chuyên môn hoặc năng khiếu đặc thù bạn sở hữu."
        >
          <div className="space-y-4">
            <ul className="space-y-2">
              {goodAt.map((item, idx) => (
                ...
                >
                  <span className="line-clamp-2 pr-2">{item}</span>
                  <button
                    onClick={() => handleRemoveItem(idx, setGoodAt)}
                    className="p-1 rounded text-stone-400 hover:text-rose-600 cursor-pointer font-sans"
                  >
                    <Trash2 size={13} />
                  </button>
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="Thêm thế mạnh của bạn..."
                className="w-full bg-stone-50 border border-stone-200 rounded-lg px-2.5 py-1.5 text-base text-stone-800 font-handwriting focus:outline-none"
              />
              <button
                onClick={() => handleAddItem("goodAt", newGoodAt, setGoodAt, setNewGoodAt)}
                className="bg-stone-900 hover:bg-stone-800 text-stone-50 rounded-lg px-3 py-1.5 text-xs font-bold cursor-pointer font-sans"
              >
                Add
              </button>
        ...
        <SectionCard 
          title={needsTitle} 
          onTitleChange={setNeedsTitle}
          subtitle="Những vấn đề của xã hội, khách hàng hoặc cộng đồng mà bạn có thể đóng góp giải quyết."
        >
          <div className="space-y-4">
            <ul className="space-y-2">
              {needs.map((item, idx) => (
                ...
                >
                  <span className="line-clamp-2 pr-2">{item}</span>
                  <button
                    onClick={() => handleRemoveItem(idx, setNeeds)}
                    className="p-1 rounded text-stone-400 hover:text-rose-600 cursor-pointer font-sans"
                  >
                    <Trash2 size={13} />
                  </button>
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="Thêm nhu cầu xã hội..."
                className="w-full bg-stone-50 border border-stone-200 rounded-lg px-2.5 py-1.5 text-base text-stone-800 font-handwriting focus:outline-none"
              />
              <button
                onClick={() => handleAddItem("needs", newNeeds, setNeeds, setNewNeeds)}
                className="bg-stone-900 hover:bg-stone-800 text-stone-50 rounded-lg px-3 py-1.5 text-xs font-bold cursor-pointer font-sans"
              >
                Add
              </button>
        ...
        {/* Card 4: What I Can Be Paid For */}
        <SectionCard 
          title={paidForTitle} 
          onTitleChange={setPaidForTitle}
          subtitle="Những công việc, dịch vụ hoặc vai trò chuyên môn thị trường sẵn sàng chi trả thu nhập cho bạn."
        >
          <div className="space-y-4">
            <ul className="space-y-2">
              {paidFor.map((item, idx) => (
                ...
                >
                  <span className="line-clamp-2 pr-2">{item}</span>
                  <button
                    onClick={() => handleRemoveItem(idx, setPaidFor)}
                    className="p-1 rounded text-stone-400 hover:text-rose-600 cursor-pointer font-sans"
                  >
                    <Trash2 size={13} />
                  </button>
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="Thêm kỹ năng kiếm tiền..."
                className="w-full bg-stone-50 border border-stone-200 rounded-lg px-2.5 py-1.5 text-base text-stone-800 font-handwriting focus:outline-none"
              />
              <button
                onClick={() => handleAddItem("paidFor", newPaidFor, setPaidFor, setNewPaidFor)}
                className="bg-stone-900 hover:bg-stone-800 text-stone-50 rounded-lg px-3 py-1.5 text-xs font-bold cursor-pointer font-sans"
              >
                Add
              </button>
      ...
      <SectionCard 
        ...
        subtitle="Tuyên bố định vị bản thân tại điểm giao thoa để định hướng cuộc sống và sự nghiệp."
      >
